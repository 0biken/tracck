...
interface Accomplishment {
  ...
  detail: string;
  confidence: number;
  status: 'pending' | 'low_confidence' | 'confirmed' | 'discarded';
  category: 'direct_achievement' | 'participation_claim';
  isEditing?: boolean;
}
...
export default function QueuePage() {
  ...
  const [items, setItems] = React.useState<Accomplishment[]>([
    {
      id: 'a1',
      platform: 'GITHUB',
      date: '2026.03.11',
    ...
    },
    {
      ...
      platform: 'TWITTER',
      date: '2026.03.01',
      claim: 'Spoke at developer meetup in Ibadan',
      detail: 'Presented a technical walk-through on building light-weight background queuing architectures for Next.js app routes.',
      confidence: 0.68, // Low confidence
      status: 'low_confidence',
      category: 'participation_claim',
    },
    {
      id: 'a4',
      platform: 'FIGMA',
      date: '2026.02.15',
      claim: 'Built UX mockup flow for portfolio builder',
      detail: 'Created flat user navigation wireframes for manual-paste and bulk file uploading modules.',
      confidence: 0.91,
      status: 'confirmed',
      category: 'direct_achievement',
    },
    ...
    setItems(prev => prev.map(item => 
      item.id === id 
        ? { 
            ...
            isEditing: false, 
            status: item.status === 'low_confidence' ? 'pending' : item.status // clear low_confidence state on edit
          } 
        : item
    ));
  };
